...
var map = new naver.maps.Map("map", {
    ...
    zoom: 10,
    size : new  naver.maps.Size($(window).width(), windowHeight), 
    mapTypeControl: true,
   ...
   var latlngTmp;
      $(xmlStr).find("juso").each(function(){         
          // alert($(this).find('entX').text());
           var tmp = new naver.maps.Point($(this).find('entX').text(),$(this).find('entY').text());
      ...
      var tm128 = naver.maps.TransCoord.fromLatLngToTM128(latlngTmp);      
       ...
       infoWindow.setContent([
                '<div style="padding:10px;min-width:100px;line-height:150%;">'+
               document.form.jibunAddr.value + '<br />'+ 
               document.form.roadAddrPart1.value +'</div>'
            ].join('\n'));
